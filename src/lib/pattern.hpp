...
namespace arisu::pattern
{
    ...
    namespace impl
    {
        ...
        Address find_simd(std::uint8_t* data, std::size_t size, const std::vector<HexData>& pattern) noexcept;
    }

    std::expected<Address, Status> find(const std::span<std::uint8_t>& data, const impl::Pattern<>& pattern) noexcept;
    std::expected<Address, Status> find(const std::span<std::uint8_t>& data, const std::vector<impl::HexData>& pattern) noexcept;
    std::expected<Address, Status> find(const std::span<std::uint8_t>& data, std::string_view pattern) noexcept;

    using type = impl::Pattern<' ', '?'>;
}

#define ARISU_MAKE_PATTERN(p) arisu::pattern::impl::make_pattern<arisu::pattern::impl::str(p)>();
